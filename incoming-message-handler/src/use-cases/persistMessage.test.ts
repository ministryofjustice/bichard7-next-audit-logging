...
describe("persistMessage", () => {
  it("should return true when server responds with 201 status code", async () => {
    const expectedSuccessfulServerResponse = '{ "statusCode": 201 }'
    jest.spyOn(axios, "post").mockResolvedValue(Promise.resolve(expectedSuccessfulServerResponse))
  ...
  it("should fail when server does not responds with 201 status code", async () => {
    const expectedFailedServerResponse = '{ "statusCode": 400 }'
