...
describe("Retry Failed Messages", () => {
  ...
  it("should retry the correct messages for the first time", async () => {
    ...
    const message = new AuditLog("External Correlation ID", new Date(Date.now() - 3600000), "Xml")
  ...
  it("should handle errors retrying messages", async () => {
    const message = new AuditLog("External Correlation ID", new Date(Date.now() - 3600), "Xml")
    message.status = "Error"
    message.events.push(
      new BichardAuditLogEvent({
        eventSource: "Dummy Event Source",
