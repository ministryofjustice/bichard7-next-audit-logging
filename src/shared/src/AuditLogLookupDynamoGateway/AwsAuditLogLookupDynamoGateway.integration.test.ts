...
const gateway = new AwsAuditLogLookupDynamoGateway(auditLogDynamoConfig, auditLogDynamoConfig.TABLE_NAME)
...
describe("AuditLogLookupDynamoGateway", () => {
  ...
  describe("fetchById", () => {
    ...
    it("should return the matching lookup item when the value is not compressed", async () => {
      const expectedLookupItem = new AuditLogLookup("Value 1", "MessageID")
      await testGateway.insertOne(auditLogDynamoConfig.TABLE_NAME, expectedLookupItem, "id")
  ...
  describe("fetchByMessageId", () => {
    it("should return an item for a specific message ID", async () => {
      ...
      await gateway.create(expectedLookupItem)
      await testGateway.insertOne(auditLogDynamoConfig.TABLE_NAME, anotherLookupItem, "id")
    ...
    it("should return items for a specific message ID when last item is provided", async () => {
      ...
      await Promise.all(
        [...Array(10).keys()].map((index) => {
          ...
          return testGateway.insertOne(auditLogDynamoConfig.TABLE_NAME, { ...item, id: `ID-${index}` }, "id")
        })
      )

      const insertedItems = (await testGateway.getAll(auditLogDynamoConfig.TABLE_NAME)).Items as AuditLogLookup[]
  ...
  describe("fetchAllByMessageId", () => {
    it("should all items for a specific message ID", async () => {
      ...
      const insertedItems = await Promise.all(
        ((await testGateway.getAll(auditLogDynamoConfig.TABLE_NAME)).Items as AuditLogLookup[]).map(async (item) => ({
          ...item,
  ...
  describe("deleteByMessageId", () => {
    it("should delete all records with a specific message ID", async () => {
      ...
      testGateway.insertOne(auditLogDynamoConfig.TABLE_NAME, otherRecord, "id")

      const result = await gateway.deleteByMessageId(messageId)
      expect(isError(result)).toBe(false)

      const recordsInDB = await testGateway.getAll(auditLogDynamoConfig.TABLE_NAME)
